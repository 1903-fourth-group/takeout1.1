...
public interface BusinessRepository extends JpaRepository<Business,Integer> {
    ...
    //查询大分类下的所有商家
    @Query(value = "SELECT * FROM business_info WHERE bus_bustype in(SELECT bus_type_id FROM business_type WHERE bus_type_pid in(SELECT bus_type_id FROM business_type WHERE bus_type_name=?1))",nativeQuery = true)
